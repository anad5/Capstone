...
def pull_ball(graph, delta_red, delta_blue):
    ...
    for node in range(num_nodes):
        ...
        graph.nodes[node]['health'].append(
            (graph.nodes[node]['red'] / graph.nodes[node]['total']))  # Update the health of each node
...
def init_urns(graph, init_red, init_blue, memory=5, memory_list=None):
    ...
    for node in range(num_nodes):
        ...
        if graph.graph['memory_flag'] == True:
            ...
            graph.nodes[node]['history'] = deque(maxlen=graph.nodes[node][
                'memory'])  # Deque to store information on which balls to be removed at what times
...
def get_scores(G, i):
    ...
    for node in range(num_nodes):
        ...
        susceptibility = G.nodes[node]['health'][
            i]  # TODO: Health is currently a list not an int, choose most recent or add new attribute of only most recent
